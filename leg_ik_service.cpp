...
for (unsigned inti=0; i<num_joints; i++){
	...
	// Build Solvers
	for (unsigned inti=0; i<num_legs; i++){
...
boolLegKinematics::loadModel(conststd::string xml) {
    KDL::Tree tree;
    KDL::Chain chain;
...
boolLegKinematics::getLegIKSolver (hexapod_msgs::GetLegIKSolver::Request &request, hexapod_msgs::GetLegIKSolver::Response &response) {
	hexapod_msgs::LegPositionStateleg_dest_pos;
	response.target_joints.clear();
	for (inti = 0; i<request.leg_number.size(); i++){
		leg_dest_pos = request.target_position[i];
		KDL::JntArrayjnt_pos_in(num_joints);
		KDL::JntArrayjnt_pos_out(num_joints);
		...
		//IK solver
		intik_valid = ik_solver_pos[request.leg_number[i]]->CartToJnt(jnt_pos_in, F_dest, jnt_pos_out);

		if (ik_valid>= 0) {
			hexapod_msgs::LegJointsStatejnt_buf;
			for (unsigned int j=0; j<num_joints; j++) {
